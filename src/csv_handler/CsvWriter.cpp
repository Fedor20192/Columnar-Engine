...
CsvWriter::CsvWriter(const std::string& filename, Parameters parameters)
    ...
    if (!file_.is_open()) {
        throw std::runtime_error("[CSVWriter]: Error opening file " + filename);
    }

    if (parameters_.delimiter == parameters_.quote) {
        throw std::runtime_error("[CSVWriter]: Delimiter and quote symbols are equal");
    }
    if (parameters_.delimiter == parameters_.linebreak) {
        throw std::runtime_error("[CSVWriter]: Delimiter and linebreak symbols are equal");
    }
    if (parameters_.quote == parameters_.linebreak) {
        throw std::runtime_error("[CSVWriter]: Quote and linebreak symbols are equal");
    }
}
