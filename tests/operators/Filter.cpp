...
TEST_CASE_METHOD(GlogFixture, "Many batch filter", "[Filter Operator]") {
    ...
    cngn::Batch batch_2(
        std::vector{
            cngn::Column(std::vector<int64_t>{9, 10, 11, 12, 13, 14, 15, 16}),
            cngn::Column(std::vector<int16_t>{1, 1, 0, 1, 1, 0, 1, 0}),
            cngn::Column(std::vector<std::string>{"q", "w", "r", "t", "y", "i", "j", "h"}),
            cngn::Column(std::vector<int32_t>{114, 122, 18, 188, 169, 167, 10, 11}),
        },
        schema);
    ...
    auto ans_1 = ans_op.value();

    REQUIRE(ans_1->RowCount() == 3);

    REQUIRE((*ans_1)[0] == cngn::Column(std::vector<int16_t>{1, 1, 1}));
    REQUIRE((*ans_1)[1] == cngn::Column(std::vector<std::string_view>{"c", "g", "h"}));
    REQUIRE((*ans_1)[2] == cngn::Column(std::vector<int32_t>{8, 0, 1}));
    ...
    auto ans_2 = ans_op.value();

    REQUIRE(ans_2->RowCount() == 5);

    REQUIRE((*ans_2)[0] == cngn::Column(std::vector<int16_t>{1, 1, 1, 1, 1}));
    REQUIRE((*ans_2)[1] == cngn::Column(std::vector<std::string_view>{"q", "w", "t", "y", "j"}));
    REQUIRE((*ans_2)[2] == cngn::Column(std::vector<int32_t>{114, 122, 188, 169, 10}));
